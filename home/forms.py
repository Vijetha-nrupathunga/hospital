...
class patientcreateform(forms.Form):
    patient_name=forms.CharField(label="Name",widget=forms.TextInput(attrs={
        'class':'form-control','maxlength':'50','placeholder':'patient name'}))
    sex=(('male','male'),('female','female'),('others','others'))
    Sex=forms.CharField(label='Sex',widget=forms.Select(attrs={'class':'form-control'},choices=sex))
    Dob=forms.DateField(label="Date of birth(in mm/dd/yyyy format)",widget=forms.DateInput(attrs={'class':'form-control','placeholder':'Date of birth'}))
    Age=forms.IntegerField(label="Age(in years)",widget=forms.TextInput(attrs={'class':'form-control','placeholder':'age'}))
    Contact=forms.CharField(label="Contact number",widget=forms.TextInput(attrs={'placeholder':'contact','class':'form-control'}))
    ...
    Blood_group=forms.CharField(label='Blood group',widget=forms.Select(attrs={'class':'form-control'},choices=blood_group))
    Password=forms.CharField(label='Password',widget=forms.PasswordInput(attrs={'placeholder':'password','class':'form-control'}))

class billform(forms.ModelForm):
    class Meta:
        model=bill
        fields=('patient','id','Date','room','ndays','report','fee','operation')
        Date=forms.DateField(label='')
